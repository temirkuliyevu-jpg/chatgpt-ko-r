"""
UstaTop Telegram Bot — Gemini AI Integratsiyali Versiya
Conversation flow, inline keyboards, SQLite DB, admin panel, AI jaroybi
"""
...
def get_ai_response(user_message: str, context_info: str = "") -> str:
    """
    Gemini AI dan javob olish
    """
    try:
        prompt = f"""
Siz UstaTop Telegram botining AI yordam beruvchisiz. 
Foydalanuvchi: {user_message}

{context_info}

Ushbu xizmatlar mavjud:
- 🔧 Santexnik (kran, quvur, kanalizatsiya ta'miri)
- ⚡ Elektrik (rozetka, sim, yoritish)
...
Foydalanuvchiga:
1. Sotuvdan oldin yaxshi maslahat ber
2. Muammoni aniqla va xizmat turi tavsiyasi ber
3. Barcha javoblar Uzbek tilida bo'lsin
4. Qisqa va tushunarli javob ber (2-3 gap)

Javob:
"""
        response = model.generate_content(prompt)
        return response.text[:500]  # Telegramda 500 ta belgiga chekla
    except Exception as e:
        logger.error(f"Gemini API xatosi: {e}")
        return "Xatolik yuz berdi. Iltimos, qaytadan urinib ko'ring."

def detect_service_with_ai(text: str) -> str | None:
    """
    AI yordamida xizmat turini aniqlash
    """
    try:
        prompt = f"""
Ushbu matn qaysi xizmatga tegishli?
Matn: "{text}"

Xizmatlar:
- santexnik (kran, quvur, unitaz, suv, kanalizatsiya)
- elektrik (tok, rozetka, chiroq, sim)
- duradgor (eshik, deraza, mebel, pol)
- rassomchi (bo'yash, devor, suvash, oboi)
- konditsioner (konditsioner, sovutish, isitish)
- plitkakor (plitka, mozaika, granit, mramor)

FAQAT xizmat nomini yozing (masalan: santexnik). Agar aniqlash mumkin bo'lmasa, "unknown" yozing.
"""
        response = model.generate_content(prompt)
        svc = response.text.strip().lower()
        if svc in SERVICES:
            return svc
        return None
    except Exception:
        # AI failed, use keyword matching
        return detect_service_keyword(text)

def detect_service_keyword(text: str) -> str | None:
    """
    Kalit so'zlar orqali xizmat turini aniqlash (backup)
    """
    t = text.lower()
...
def categorize_with_ai(description: str, service_id: str) -> str:
    """
    AI yordamida muammoni kategoriyalash
    """
    try:
        prompt = f"""
Xizmat turi: {SERVICES.get(service_id, ('', 'Noma\\'lum'))[1]}
Muammo: {description}

Muammoning kategoriyasini qisqacha nomla (masalan: "Kran ta'miri", "Rozetka o'rnatish").
FAQAT kategoriya nomini yozing, boshqa ma'lumot yozmang.
"""
        response = model.generate_content(prompt)
        return response.text.strip()[:50]
    except Exception:
        return f"{SERVICES.get(service_id, ('','Umumiy'))[1]} — Umumiy muammo"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "🤖 AI yordamchiga ham savol berish mumkin!\n\n"
...
async def get_description(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # AI yordamida kategoriyalash
    svc_id = context.user_data.get("service_type", "")
    cat = categorize_with_ai(text, svc_id)
    context.user_data["category"] = cat

    await update.message.reply_text(
        f"✅ Qabul qilindi! 🤖 AI tahlil: '{cat}'\n\n"
        "👤 *Ismingizni* yozing:",
        parse_mode="Markdown",
...
async def ai_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """AI yordamchi - foydalanuvchining savollariga javob berish"""
    msg = update.message.text
    
    # Savol qiling tugmasi
    await update.message.reply_text(
        "🤖 *AI Yordam Beruvchi*\n\n"
        "Muammongiz haqida savol yozing, men sizga maslahat beraman.\n\n"
        "_Masalan: 'Kran buzilganda nima qilish kerak?' yoki 'Elektr qisqa tutashuvi xavfimi?'_",
        parse_mode="Markdown",
        reply_markup=ReplyKeyboardRemove(),
    )
...
async def ai_response_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # AI dan javob olish
    typing_msg = await update.message.reply_text("⏳ Javob tayyorlanmoqda...")
...
async def info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "🌐 Sayt: usatop.uz\n\n"
        "🤖 *AI Yordamchi* — barcha savollarga tez javob beradi!",
        parse_mode="Markdown",
        reply_markup=main_menu_keyboard(),
    )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📋 Buyurtma berish":
        ...
        return await ai_chat(update, context)
    elif text == "ℹ️ Ma'lumot":
        await info(update, context)
    elif text == "📞 Aloqa":
        await contact(update, context)
    else:
        # Auto-detect service from free text va AI bilan
        svc = detect_service_with_ai(text)
        if svc:
            context.user_data["description"] = text
            context.user_data["service_type"] = svc
            icon, name = SERVICES[svc]
            await update.message.reply_text(
                f"🤖 AI muammongizni tushunadim!\n{icon} *{name}* xizmati kerak ko'rinadi.",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(f"✅ {icon} {name}", callback_data=f"svc_{svc}"),
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            AI_CHAT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, ai_response_handler),
            ],
        },
        fallbacks=[
    ...
    logger.info("🤖 UstaTop bot (Gemini AI integratsiyali) ishga tushdi!")
